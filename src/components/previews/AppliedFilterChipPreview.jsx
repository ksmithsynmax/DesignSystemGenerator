...
export default function AppliedFilterChipPreview({
  brands,
  brandId,
  variant = "filled",
  size,
  ...
  const tokens = COMPONENT_TOKENS.appliedfilterchip;
  ...
  const chipColorKeys = (prop) => {
    ...
    const make = (useV, useC, useIa, useIt) => {
      const base = useV ? `appliedfilterchip-${variant}-${prop}` : `appliedfilterchip-${prop}`;
      let s = "";
  ...
  const focusRing = resolveFirst(["appliedfilterchip-focus-ring"]);
  const warningColor = resolveFirst(["appliedfilterchip-selective-inactive-warning-color"]);
  const warningBorderColor = resolveFirst(["appliedfilterchip-selective-inactive-warning-border"]);
  const warningIconColor = resolveFirst(["appliedfilterchip-selective-inactive-warning-icon"]);
  const selectiveInactiveBorderStyle =
    resolveDimension(brands, brandId, "appliedfilterchip-selective-inactive-border-style") || "dashed";

  // Sub-label + remove ("×") colors
  const subLabelColor = resolveFirst([
    checked ? "appliedfilterchip-sublabel-color-selected" : "appliedfilterchip-sublabel-color",
    "appliedfilterchip-sublabel-color",
  ]);
  // Remove ("×") color. Selected and Selective Inactive have distinct colors,
  // each of which can vary by interaction state (hover/pressed). Resolve most
  // specific first, degrading Selective Inactive → Selected → base.
  const removeColor = isDisabled
    ? textColor
    : resolveFirst([
        checked && isInactive && interaction ? `appliedfilterchip-remove-color-selected-inactive-${interaction}` : null,
        checked && isInactive ? "appliedfilterchip-remove-color-selected-inactive" : null,
        checked && interaction ? `appliedfilterchip-remove-color-selected-${interaction}` : null,
        checked ? "appliedfilterchip-remove-color-selected" : null,
        "appliedfilterchip-remove-color",
      ].filter(Boolean));
  const chipIconStrokeWidth = resolveDimension(brands, brandId, "appliedfilterchip-icon-stroke-width", size);

  // Resolve dimensions
  const chipPaddingX = resolveDimension(brands, brandId, "appliedfilterchip-padding-x", size);
  const chipPaddingY = resolveDimension(brands, brandId, "appliedfilterchip-padding-y", size);
  const chipIconSize = resolveDimension(brands, brandId, "appliedfilterchip-icon-size", size);
  const chipFontSize = resolveDimension(brands, brandId, "appliedfilterchip-font-size", size);
  const chipFontFamily = resolveDimension(brands, brandId, "appliedfilterchip-font-family");
  const chipFontWeightDefault = resolveDimension(brands, brandId, "appliedfilterchip-font-weight");
  const chipFontWeightSelected = resolveDimension(brands, brandId, "appliedfilterchip-font-weight-selected");
  const chipFontWeightInactive = resolveDimension(brands, brandId, "appliedfilterchip-font-weight-inactive");
  const chipFontWeightSelectedInactive = resolveDimension(brands, brandId, "appliedfilterchip-font-weight-selected-inactive");
  ...
  const chipLineHeight = resolveDimension(brands, brandId, "appliedfilterchip-line-height", size);
  const chipRadius = resolveDimension(brands, brandId, "appliedfilterchip-radius", radius || size);
  const chipSpacing = resolveDimension(brands, brandId, "appliedfilterchip-spacing", size);
  const chipBorderWidth = resolveDimension(brands, brandId, "appliedfilterchip-border-width");
  const subLabelFontSize = resolveDimension(brands, brandId, "appliedfilterchip-sublabel-font-size", size);
  const subLabelLineHeight = resolveDimension(brands, brandId, "appliedfilterchip-sublabel-line-height", size);
  const subLabelSpacing = resolveDimension(brands, brandId, "appliedfilterchip-sublabel-spacing");
  const removeSize = resolveDimension(brands, brandId, "appliedfilterchip-remove-size", size);
  const removeStrokeWidth = resolveDimension(brands, brandId, "appliedfilterchip-remove-icon-stroke-width", size);
